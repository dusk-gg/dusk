...
At Rune, we love all types of games—from 2D to isometric to 3D—and we want you to build with the tools you prefer on our platform. Creating 3D games brings added complexities, like camera controls and character movement, which can be challenging to make feel natural, even in single-player settings. The examples below serve as a straightforward reference for building 3D games with [Three.js](https://threejs.org) on the Rune platform.

![](/img/blog/callouts/Three.js.gif)

## Approach

Building a 3D game is more complex than a 2D game, though the multiplayer components remain essentially the same when using the Rune SDK. In this tech demo, we'll cover key aspects of building a multiplayer 3D game:
...
Creating a responsive 3D character and camera controller can be challenging. For instance, our recent release, *MeatSuits*, features a controller inspired by Roblox. This demo distills the basics of that controller.
...
Note that we’re also recording the current `cameraAngle` as part of the controls. This allows the movement code in the logic (see below) to interpret the inputs accurately.
